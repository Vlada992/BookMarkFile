...
function fetchBookmarks(){
    ...
    //for (var i = 0; i < bookmarks.length; i++){
        ...
        var name = [];
        name.push(bookmarks[bookmarks.length -1].name);

        console.log(name);
        var url = bookmarks[bookmarks.length -1].url;
        //console.log('a THUMBNAL:', document.getElementsByClassName('thumbnail'))


  var takeImgDom = document.getElementById('imgIdBuk');
  
  console.log(takeImgDom)
  takeImgDom.setAttribute('src', 'http://api.screenshotlayer.com/api/capture?access_key=99359d2c1a52b834f4115b4e13a71262&url=' + url + '&width=400')
  var takeImgDom1 = document.getElementById('imgIdFav');
  takeImgDom1.setAttribute('src', 'https://plus.google.com/_/favicon?domain_url=' + url);
  var takeText = document.getElementById('btnH3Id');
  takeText.innerHTML +=  `&nbsp; ${name}`;
  console.log(takeText);
  var takeBtn1 = document.getElementById('btnTake1');
  takeBtn1.setAttribute('href', url);
  var takeI = document.getElementById('takeIconI');
  takeI.addEventListener('click', () => {
  	  callIt(url);
  })
  var takeBtn2 = document.getElementById('btnTake2');
...
function callIt(takeUrl){
	console.log('TAKEURL je:', takeUrl);
    var image = new Image();
